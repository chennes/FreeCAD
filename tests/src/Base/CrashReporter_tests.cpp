...
/// A genuinely fatal fault code that carries no faulting data address: POSIX leaves si_addr
/// undefined for SIGABRT, and a Windows stack overflow reports no data address.
constexpr std::uint32_t addressFreeFaultCode()
{
#ifdef FC_OS_WIN32
...
public:
    static std::vector<char> createGoodCrashReport(
        std::uint32_t faultCode = addressCarryingFaultCode()
    );
...
std::vector<char> CrashReporterTests::createGoodCrashReport(std::uint32_t faultCode)
{
    ...
    // Always planted, whatever the fault code: the Writer stores si_addr unconditionally, so
    // suppressing it for codes that have no data address is the Reader's job, and a test that
    // planted zero here could not tell suppression from an absent value.
    ...
    header.osID = Base::CrashReporter::OS::BSD;
#endif
...
#elif defined(FC_OS_BSD)
    EXPECT_EQ(report.osID, Base::CrashReporter::OS::BSD);
#endif
...
TEST_F(CrashReporterTests, parseDropsFaultAddressForCodeThatHasNone)  // NOLINT
{
    ...
    {
        std::ofstream ofs(fcrashPath, std::ios::binary);
        auto buffer = createGoodCrashReport(addressFreeFaultCode());
        ofs.write(buffer.data(), static_cast<std::streamsize>(buffer.size()));
    }
    auto report = Base::CrashReporter::parse(fcrashPath.string());
    ...
    EXPECT_EQ(report.code, addressFreeFaultCode());
...
TEST_F(CrashReporterTests, parseNamesUnrecognizedFaultCodeWithItsHexValue)  // NOLINT
{
    ...
    // this, don't change it without good cause.
...
TEST_F(CrashReporterTests, scanDoesNotSetOSVersionWhenNotPresent)  // NOLINT
{
    ...
    Base::CrashReporter::Manager::scan(tempDir.string(),
        {}
        /*, No version string set here: defaults to an empty optional*/ );
    const auto& reports = Base::CrashReporter::Manager::reports();
    ASSERT_EQ(reports.size(), 1U);
...
{
    EXPECT_FALSE(Base::CrashReporter::faultAddressIsMeaningful(addressFreeFaultCode()));
}
...
TEST_F(CrashReporterTests, faultAddressIsMeaningfulReturnsTrueForCodeWithAddress)  // NOLINT
{
    ...
    EXPECT_TRUE(Base::CrashReporter::faultAddressIsMeaningful(addressCarryingFaultCode()));
}

TEST_F(CrashReporterTests, describeFaultCodeGivesValidResultForValidCode)  // NOLINT
{
    // Spot check (don't bother with an exhaustive test, it's tautological)
    auto description = Base::CrashReporter::describeFaultCode(addressCarryingFaultCode());
...
{
    auto description =
        Base::CrashReporter::describeFaultCode(std::numeric_limits<std::uint32_t>::max());
