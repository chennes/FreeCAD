...
ShapeBinder::~ShapeBinder()
{
    try {
        ...
        // TODO(chennes): When in debug, this should die
    }
}
                                        ...
                                        std::vector<std::string>& subobjects)
{
    ...
    if (obj) {
        // if we have no sub-shape we use the whole shape
        if (subs[index].empty()) {
            return;
        }

        // collect all sub-shapes for the object
        for (index = 0; index < objs.size(); index++) {
            // we only allow sub-shapes from a single Part::Feature
            if (objs[index] != obj) {
                continue;
            }
...
SubShapeBinder::SubShapeBinder()
{
    ...
    static std::array<const char*, 4> JoinTypeEnum = {"Arcs", "Tangent", "Intersection", nullptr};
    OffsetJoinType.setEnums(JoinTypeEnum.data());
    ...
    static std::array<const char*,4> BindModeEnum = {"Synchronized", "Frozen", "Detached", nullptr};
    BindMode.setEnums(BindModeEnum.data());
    ...
    static std::array<const char*,4> BindCopyOnChangeEnum = {"Disabled", "Enabled", "Mutated", nullptr};
    BindCopyOnChange.setEnums(BindCopyOnChangeEnum.data());
    ADD_PROPERTY_TYPE(
        BindCopyOnChange,
        ((long)0),
        "Base",
...
App::DocumentObject* SubShapeBinder::getSubObject(const char* subname,
                                                  ...
                                                  int depth) const
{
    if (auto sobj = Part::Feature::getSubObject(subname, pyObj, mat, transform, depth)) {
        return sobj;
    }
...
void SubShapeBinder::update(SubShapeBinder::UpdateOption options)
{
    ...
    bool forced = Shape.getValue().IsNull() || ((options & UpdateForced) != 0);
    bool init = !forced && ((options & UpdateForced) != 0);
                    ...
                    const auto copyPropertyValues =
                        [this, &recomputeCopy, &props, copied](const bool to_support) {
                            for (auto prop : props) {
                                ...
                                // we only copy read-only and output properties from support to
                                // binder
                                if (!to_support
...
>>>>>>> Stashed changes
    ...
    if (!init) {
        ...
        for (auto& shape : shapes) {
            ++idx;
            if ((shape.Hasher != nullptr) && (shape.getElementMapSize() != 0u)
...
void SubShapeBinder::collapseGeoChildren()
{
    ...
    if (!removes.empty() || !newVals.empty()) {
