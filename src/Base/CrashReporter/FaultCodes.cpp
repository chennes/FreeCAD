...
{
    const auto description = std::ranges::find(
        faultCodeToDescription, code, &FaultDescription::code);
...
std::optional<FaultDescription> describeFaultCode(std::uint32_t code)
{
    ...
    return signal != faultCodeToDescription.end() ? std::optional {*signal} : std::nullopt;
}

std::string faultCodeName(std::uint32_t code)
{
    const auto description = describeFaultCode(code);
    if (description.has_value()) {
        return std::string(description.value().name);
    }
