...
ElementMapPtr ComplexGeoData::getElementMap(bool flush) const {
    if (flush) {
        flushElementMap();
    }
    return _elementMap;
...
void ComplexGeoData::Save(Base::Writer &writer) const {
    ...
    _elementMap->save(writer.beginCharStream() << '\n');
...
void ComplexGeoData::Restore(Base::XMLReader &reader) {
    ...
    bool newtag = false;
    if (reader.hasAttribute("new") &&
        reader.getAttributeAsInteger("new") > 0) {
