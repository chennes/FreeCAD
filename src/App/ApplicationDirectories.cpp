...
ApplicationDirectories::ApplicationDirectories(std::map<std::string,std::string> &config) :
    _currentVersion(std::make_tuple(std::stoi(config["BuildVersionMajor"]), std::stoi(config["BuildVersionMinor"])))
{
...
void ApplicationDirectories::configurePaths(std::map<std::string,std::string>& mConfig)
{
    ...
    fs::path versionedDataPath = mostRecentConfigFromBase(data);
    _userAppData = versionedDataPath;
    mConfig["UserAppData"] = Base::FileInfo::pathToString(versionedDataPath) + PATHSEP;


    // User config path
    //
    fs::path config = findPath(configHome, customHome, subdirs, true);
    fs::path versionedConfigPath = mostRecentConfigFromBase(config);
    _userConfig = versionedConfigPath;
    mConfig["UserConfigPath"] = Base::FileInfo::pathToString(versionedConfigPath) + PATHSEP;
    ...
    fs::path versionedMacroPath = mostRecentConfigFromBase(macro);
    _userMacro = versionedMacroPath;
    mConfig["UserMacroPath"] = Base::FileInfo::pathToString(versionedMacroPath) + PATHSEP;
}
...
fs::path ApplicationDirectories::findHomePath(const char* sCall)
{
    ...
    return FileInfo::stringToPath(homePath);
}
...
fs::path ApplicationDirectories::findHomePath(const char* sCall)
{
    ...
    if (!Py_IsInitialized()) {
        ...
        if (const auto buf = new char[++sz]; _NSGetExecutablePath(buf, &sz) == 0) {
            ...
            if (path) {
                ...
                return FileInfo::stringToPath(TempHomePath);
            }
        } else {
            delete [] buf;
        }
    ...
    return FileInfo::stringToPath(sCall);
}
